...
func (m *Module) wizardTCP(body wizard) (string, error) {
	ip := "192.168.15.202"
	if body.IP != "" {
		ip = body.IP
	}
	...
	case 0:
		...
		_, err := m.grpcMarshaller.WizardNewNetworkDevicePoint("modbus", &net, &dev, &pnt)
		if err != nil {
	...
	case 1:
		...
		m.modbusDebugMsg("Created a Network")
		...
		for j := 1; j < 4; j++ {
			...
			if err != nil {
				m.modbusErrorMsg(fmt.Sprintf("device creation failure: %s", err))
			}
			m.modbusDebugMsg("Created a Device: ", dev)
			...
			point, err := m.grpcMarshaller.CreatePoint(&pnt)
			if err != nil {
				return "", err
			}
			...
			m.modbusDebugMsg("Created a Point for Consumer", pnt)
	...
	case 5:
		if body.NameArg != "" && body.AddArg > 0 {
			...
			if err != nil || net == nil {
				...
				m.modbusDebugMsg("Network already exists")
			}

			dev, exists := m.grpcMarshaller.DeviceNameExistsInNetwork(body.NameArg, net.UUID)
			if err != nil || dev == nil || !exists {
				...
				m.modbusDebugMsg("Device already exists")
			}
			...
			for _, point := range pointsArray {
				...
				_, err = m.addPoint(pnt)
				if err != nil {
					m.modbusErrorMsg(fmt.Sprintf("point creation failure: %s", err))
				}
				m.modbusDebugMsg("Created a Point for Consumer", pnt)
			}
...
func (m *Module) wizardSerial(body wizard) (string, error) {
	sp := "/dev/ttyUSB0"
	if body.SerialPort != "" {
		sp = body.SerialPort
	}
	...
	pntRet, err := m.grpcMarshaller.WizardNewNetworkDevicePoint("modbus", &net, &dev, &pnt)
	if err != nil {
		return "error: on rubix-os add modbus serial network wizard", err
	}

	m.modbusErrorMsg(pntRet, err)
