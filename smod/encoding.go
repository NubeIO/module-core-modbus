...
func bytesToUint16(endianness Endianness, in []byte) (out uint16) {
	if math.Mod(float64(len(in)), 2) != 0 {
		log.Error("MODBUS CONVERSION ERROR bytesToUint16(): length of input []byte is less than 2")
		return uint16(0)
	}
...
func bytesToUint16s(endianness Endianness, in []byte) (out []uint16) {
	if math.Mod(float64(len(in)), 2) != 0 {
		log.Error("MODBUS CONVERSION ERROR bytesToUint16s(): length of input []byte is less than 2")
...
func bytesToInt16s(endianness Endianness, in []byte) (out []int16) {
	if math.Mod(float64(len(in)), 2) != 0 {
		log.Error("MODBUS CONVERSION ERROR bytesToInt16s(): length of input []byte is less than 2")
...
func bytesToUint32s(endianness Endianness, wordOrder WordOrder, in []byte) (out []uint32) {
	...
	if math.Mod(float64(len(in)), 4) != 0 {
		log.Error("MODBUS CONVERSION ERROR bytesToUint32s(): length of input []byte is less than 4")
...
func bytesToInt32s(endianness Endianness, wordOrder WordOrder, in []byte) (out []int32) {
	var i32 int32
	if math.Mod(float64(len(in)), 4) != 0 {
		log.Error("MODBUS CONVERSION ERROR bytesToInt32s(): length of input []byte is less than 4")
...
func bytesToUint64s(endianness Endianness, wordOrder WordOrder, in []byte) (out []uint64) {
	...
	if math.Mod(float64(len(in)), 8) != 0 {
		log.Error("MODBUS CONVERSION ERROR bytesToUint64s(): length of input []byte is less than 8")
...
func bytesToInt64s(endianness Endianness, wordOrder WordOrder, in []byte) (out []int64) {
	var i64 int64
	if math.Mod(float64(len(in)), 8) != 0 {
		log.Error("MODBUS CONVERSION ERROR bytesToInt64s(): length of input []byte is less than 8")
